...
subsection{*Representing IPv4 Adresses*}
  ...
  lemma nat_of_ipv4addr_slice_ipv4addr_of_nat: 
    ...
    by (smt Divides.pos_mod_sign zpower_int)
  ...
  "\<lbrakk> a < 256; b < 256; c < 256; d < 256 \<rbrakk> \<Longrightarrow> dotteddecimal_of_ipv4addr (ipv4addr_of_dotteddecimal (a,b,c,d)) = (a,b,c,d)"
    ...
    (*only the shiftright cases left*)
    apply(simp add: ipv4addr_of_nat_def word_of_nat)
    apply(simp add: nat_of_ipv4addr_def unat_def)
  ...
  text{*previous and next ip adresses, without wrap around*}
...
subsection{*IP ranges*}
  ...
  lemma ipv4addr_or_NOT4294967296: "(foo::ipv4addr) OR -4294967296 = foo"
    proof -
      let ?null="-4294967296::ipv4addr"
      have "?null = 0" by simp
  ...
  lemma ipv4range_set_from_bitmask_32: "ipv4range_set_from_bitmask foo 32 = {foo}"
    ...
    apply(simp_all add: ipv4addr_or_NOT4294967296)
    done
  ...
  (* apply(induction rs)
    apply(simp) 
    apply(subst ipv4range_to_list.simps(1))
    apply(subst ipv4range_to_set.simps(2))
    apply(subst map_append, subst set_append)
  ...
  (*
  fun ipv4range_merge_rule where
    "ipv4range_merge_rule (IPv4Range s e) (IPv4Range sm em # rs) = ipv4range_merge_rule (if s > ip_next em \<or> e < ip_prev sm
      then (IPv4Range s e) else (IPv4Range (min sm s) (max em e))) rs" |
    "ipv4range_merge_rule r [] = r" |
    "ipv4range_merge_rule _ _ = undefined"
  lemma ipv4range_merge_rule_set_eq[simp]: "ipv4rangelist_union_free rs \<Longrightarrow>
    ipv4range_to_set (ipv4range_merge_rule (IPv4Range s e) rs) \<union> (\<Union>set (map ipv4range_to_set rs)) = ipv4range_to_set (IPv4Range s e) \<union> (\<Union>set (map ipv4range_to_set rs))"
    apply(induction rs arbitrary: s e) apply(simp)
    apply(case_tac a) prefer 2 apply simp
    apply(rename_tac s e sm em)
    apply(simp only: ipv4range_merge_rule.simps)
    apply(case_tac "s > ip_next em \<or> e < ip_prev sm") apply(simp_all only: if_False if_True) apply(auto)[1]
    apply(case_tac "s \<le> sm", case_tac [!] "e \<ge> em")
    apply(simp, metis (hide_lams, no_types) max.order_iff min_max.inf_absorb2 min_max.sup_commute sup_left_commute)
    proof -
      case goal1
      have "ipv4rangelist_union_free rs" using goal1(2) by simp
      note mIH = goal1(1)[OF this]
      from goal1(4) have "s \<le> ip_next em" "ip_prev sm \<le> e" by auto
      have "ipv4range_to_set (IPv4Range s e) \<union> \<Union>set (map ipv4range_to_set (IPv4Range sm em # rs)) = 
        ipv4range_to_set (IPv4Range s e) \<union> ipv4range_to_set (IPv4Range sm em) \<union> \<Union>set (map ipv4range_to_set rs)" by force

      have "ipv4range_to_set (ipv4range_merge_rule (IPv4Range s em) rs) \<union> \<Union>set (map ipv4range_to_set (IPv4Range sm em # rs)) = 
        ipv4range_to_set (ipv4range_merge_rule (IPv4Range s em) rs) \<union> ipv4range_to_set (IPv4Range sm em) \<union> \<Union>set (map ipv4range_to_set rs)" by force
      moreover have "\<dots> = ipv4range_to_set (ipv4range_merge_rule (IPv4Range s e) rs) \<union> ipv4range_to_set (IPv4Range sm em) \<union> \<Union>set (map ipv4range_to_set rs)" 
        (*using goal1(6) ipv4range_merge_rule_mono1[OF order_less_imp_le[OF linorder_class.not_leE[OF goal1(6)]], of s rs]
        apply(subst set_eq_subset) apply(rule) prefer 2 apply force*)
        sorry
      from mIH[of s e] have "ipv4range_to_set (ipv4range_merge_rule (IPv4Range s em) rs) \<union> \<Union>set (map ipv4range_to_set (IPv4Range sm em # rs)) =
        ipv4range_to_set (IPv4Range s e) \<union> \<Union>set (map ipv4range_to_set (IPv4Range sm em # rs))"  sorry
      with goal1(5) goal1(6) show ?case
        by(metis linear min_max.inf_absorb1 min_max.inf_commute min_max.sup_absorb2 min_max.sup_commute)
    oops*)

  (*fun ipv4range_merge_helper where
    "ipv4range_merge_helper [] m = m" |
    "ipv4range_merge_helper ((IPv4Range s e) # rs) m = ipv4range_merge_helper rs
      (if ipv4range_subset_some (IPv4Range s e) m 
        then ipv4range_merge_helper rs m 
        else (IPv4Union (ipv4range_merge_rule (IPv4Range s e) rs) m))" |
    "ipv4range_merge_helper _ _ = undefined"
  lemma ipv4range_merge_helper_set_eq[simp]: "ipv4rangelist_union_free r \<Longrightarrow> 
    ipv4range_to_set (ipv4range_merge_helper r m) = (\<Union>set (map ipv4range_to_set r)) \<union> ipv4range_to_set m"
    apply(induction r arbitrary: m)
    apply(simp_all)
    apply(case_tac a) prefer 2 apply(simp) apply(rename_tac a r m s e) 
    apply(simp only: ipv4range_merge_helper.simps)
    apply(case_tac "ipv4range_subset_some a m") apply(simp)
    apply(metis ipv4range_subset_some_set_eq ipv4range_to_set.simps(1) le_iff_sup sup.coboundedI2 sup_assoc)
    apply(simp only: if_False ipv4range_to_set.simps(2))
    (*using ipv4range_merge_rule_set_eq by fastforce*) oops

  fun ipv4range_merge where
    "ipv4range_merge m = ipv4range_optimize_empty (ipv4range_merge_helper (ipv4range_to_list m) (IPv4Range 2 1))"
  lemma ipv4range_merge_set_eq[simp]: "ipv4range_to_set (ipv4range_merge r) = ipv4range_to_set r"
    (*using ipv4range_to_list_set_eq ipv4range_merge_helper_set_eq ipv4range_to_list_union_free by simp*) oops

  value "ipv4range_merge (IPv4Union (IPv4Range 2 3) (IPv4Range 1 1))"*)
(*
  fun ipv4range_ where
    "ipv4range_ (IPv4Range s e)" |
    "ipv4range_ (IPv4Union r1 r2)"
*)
