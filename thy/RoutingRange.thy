...
fun viewrange :: "ipv4range \<Rightarrow> ((nat \<times> nat \<times> nat \<times> nat) \<times> (nat \<times> nat \<times> nat \<times> nat)) list" where 
"viewrange (IPv4Range l h) = [(dotteddecimal_of_ipv4addr l, dotteddecimal_of_ipv4addr h)]" |
"viewrange (IPv4Union a b) = viewrange a @ viewrange b"

fun range_destination :: "prefix_routing \<Rightarrow> ipv4rq \<Rightarrow> (ipv4rq \<times> port set) list" where
"range_destination [] rg = (if ipv4rq_empty rg then [] else [(rg, {})])" |
"range_destination (r # rs) rg = (
  let rpm = range_prefix_match (routing_match r) rg in (let m = fst rpm in (let nm = snd rpm in (
    (if ipv4rq_empty m  then [] else [ (m, routing_action r) ]) @ 
    (if ipv4rq_empty nm then [] else range_destination rs nm)
))))"

lemma range_destination_eq: 
  "{ (ipv4rq_to_set r, ports)|r ports. (r, ports) \<in> set (range_destination rtbl rg)} = ipset_destination rtbl (ipv4rq_to_set rg)"
  apply(induction rtbl arbitrary: rg)
   apply(simp)
  apply(simp only: Let_def range_destination.simps ipset_destination.simps)
  apply(case_tac "fst (ipset_prefix_match (routing_match a) (ipv4rq_to_set rg)) = {}")
...
definition "rr_to_sr r \<equiv> set (map (\<lambda>(x,y). (ipv4rq_to_set x, y)) r)"

lemma range_destination_eq2:
  "ipv4rq_to_set rg = rS \<Longrightarrow>
  ...
  proof -
    case goal1
    let ?maf = "(\<lambda>(x, y). (ipv4rq_to_set x, y))"
    have ne: "ipv4rq_to_set (fst (range_prefix_match (routing_match a) rg)) \<noteq> {}"
             "ipv4rq_to_set (snd (range_prefix_match (routing_match a) rg)) \<noteq> {}"
      using goal1(3) goal1(4) goal1(2) by simp_all
    have *: "snd (ipset_prefix_match (routing_match a) rS) = ipv4rq_to_set (snd (range_prefix_match (routing_match a) rg))"
...
lemma in_range_rel: "in_rel (range_rel r) x y = (\<exists>rg. ipv4rq_element x rg \<and> (rg,y) \<in> set r)"
  unfolding ipv4rq_element_set_eq in_rel_def range_rel_def by auto
...
lemma range_destination_correct:
  ...
  shows "(packet_routing_table_semantics rtbl packet = ports) \<longleftrightarrow> in_rel (range_rel (range_destination rtbl ipv4rq_UNIV)) (dst_addr packet) ports"
  unfolding packet_routing_table_semantics_def ipset_destination_correct[OF v_pfx UNIV_I] ipv4rq_UNIV_set_eq[symmetric] range_destination_eq[symmetric] range_rel_def ipset_rel_def
  by simp blast

fun map_of_ranges where
"map_of_ranges [] = (\<lambda>x. undefined)" |
"map_of_ranges ((a,b)#rs) = (\<lambda>x. if ipv4rq_element x a then b else map_of_ranges rs x)"
...
apply(case_tac "\<not>ipv4rq_element a aa")
apply(unfold ipv4rq_element_set_eq)
apply auto[1]
apply clarsimp
apply(rule)+
apply simp
...
proof -
  fix aa ba
  assume "(map_of_ranges r a = b) = (\<exists>rg. (rg, b) \<in> set r \<and> a \<in> ipv4rq_to_set rg)"
  assume "a \<in> ipv4rq_to_set aa"
  assume " \<exists>rg. (rg = aa \<and> b = ba \<or> (rg, b) \<in> set r) \<and> a \<in> ipv4rq_to_set rg"
  show "ba = b"*)

lemma range_left_side_nonempty: "x \<in> set (map fst (range_destination rtbl rg)) \<Longrightarrow> \<not> ipv4rq_empty x"
proof -
  case goal1
  have "\<exists>S. S = ipv4rq_to_set rg" by simp then guess S ..
...
definition "range_left_reduce \<equiv> list_left_reduce list_to_ipv4rq"
lemmas range_left_reduce_set_eq = list_left_reduce_set_eq[OF list_to_ipv4rq_set_eq rr_to_sr_def, 
...
lemma reduced_range_destination_eq1: (* equality that was first proven. *)
  "{ (ipv4rq_to_set r, ports)|r ports. (r, ports) \<in> set (reduced_range_destination rtbl rg)} = reduced_ipset_destination rtbl (ipv4rq_to_set rg)"
